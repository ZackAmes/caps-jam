...
import { CapsV4Adapter } from './game/v4';
...
async function main() {
  ...
  const adapter = new CapsV4Adapter(provider, account, config.actionsAddress);
  await adapter.checkCompatibility();
  const identity = `${chainId}:${BigInt(config.actionsAddress)}:${BigInt(config.address)}:rules4`;
